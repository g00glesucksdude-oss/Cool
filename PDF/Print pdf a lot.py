...
def split_pdf(file_path, chunk_size=95, reverse=True, status_callback=None):
    ...
    # Build list of pages in desired order
    ...
    for chunk_index, start in enumerate(range(0, total_pages, chunk_size)):
        ...
        chunk_pages = page_numbers[start:end]  # these are the actual PDF page indices

        odd_writer = PdfWriter()
        even_writer = PdfWriter()

        # Split by POSITION within the chunk (1st page = odd, 2nd = even, etc.)
        for position, page_index in enumerate(chunk_pages):
            if position % 2 == 0:  # 0-indexed position: 0,2,4... = odd pages
                odd_writer.add_page(reader.pages[page_index])
            else:                  # 1,3,5... = even pages
                even_writer.add_page(reader.pages[page_index])

        # Use chunk number for filename so it's clear what order to print
        ...
        chunks_created += 1
        if status_callback:
            status_callback(f"Chunk {chunk_label} done ({end}/{total_pages} pages)...")
...
def select_and_split():
    ...
    try:
        ...
        status_var.set(f"Done! {total_pages} pages across {chunks} chunk(s).")
        messagebox.showinfo(
            "Done",
            f"Split complete!\n\n"
            ...
            f"Print order per chunk:\n"
            f"  1. Print odds → 2. Print evens → 3. Stack\n"
            f"  Repeat for each chunk."
        )
...
tk.Label(root, text="PDF Odd/Even Splitter", font=("Helvetica", 14, "bold")).pack(padx=16, pady=(16, 4))
