...
def detect_overlap_regions_osd(audio_path: str, hf_token: str, min_duration_on: float = 0.3,
                               min_duration_off: float = 0.1) -> list:
    """Run pyannote OverlappedSpeechDetection (no speaker IDs).
    ...
    try:
        ...
        raise RuntimeError(
            "OverlappedSpeechDetection not available in this pyannote.audio version. "
            "Upgrade pyannote.audio or skip overlap resolution."
        )

    print("[+] Loading overlapped-speech detection (pyannote/segmentation-3.0)...")
    try:
        try:
        ...
        raise RuntimeError(
            f"Could not load pyannote/segmentation-3.0 ({e}). "
            "Accept access at https://huggingface.co/pyannote/segmentation-3.0"
        ) from e
    ...
    try:
        ...
        print(f"[+] OSD found {len(regions)} overlap region(s).")
        return regions
    finally:
        del osd, seg_model
        free_memory(aggressive=True)
...
def auto_transcribe_missing(manifest_path: str, transcripts_dirs: list, backend: str, style: str) -> int:
    ...
    When backend is Groq: any incomplete local (faster-whisper) transcripts are
    automatically deleted first so those segments are re-done by Groq instead
    of being treated as already finished.
    ...
    # If switching to Groq after a partial local run, drop incomplete local
    # transcripts so they show up as missing and get re-transcribed cleanly.
    if backend == "groq":
